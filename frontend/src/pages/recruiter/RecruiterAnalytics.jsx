...
export default function RecruiterAnalytics() {
  ...
  const { jobs, applications, interviews, hiring, topJobs, weeklyTrend } = analytics;
  ...
  return (
    <div className="page">
      <div className="container">
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {Object.entries(applications.byStatus).map(([status, count]) => (
                <div key={status} style={{
                  ...
                  <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', textTransform: 'capitalize' }}>{status}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {topJobs.length === 0 && weeklyTrend.length === 0 && (
          <div className="card">
            <div className="empty-state">
              <div className="icon">📊</div>
              <h3>No data yet</h3>
              <p>Analytics will appear once you have job postings and applications.</p>
            </div>
          </div>
        )}
      </div>
